...
const DROP = 'urn:kbase:concept/meadow-farmers-market';
...
describe('mergeAliasValues', () => {
  it('preserves the dropped entity name that a merge would otherwise reject', () => {
    const statements = [
      ...
      st(DROP, RDFS_LABEL, 'Meadow Farmers Market'),
    ];
    expect(mergeAliasValues(KEEP, DROP, statements, 'Meadow Growers Market')).toContain(
      'Meadow Farmers Market',
    );
  });
  ...
  it('never repeats the primary label as an alias', () => {
    ...
    expect(mergeAliasValues(KEEP, DROP, statements, 'Meadow Growers Market')).not.toContain(
  ...
  it('treats case and whitespace differences as the same name, not a synonym', () => {
    const statements = [
      st(KEEP, RDFS_LABEL, 'Meadow Growers Market'),
      st(DROP, RDFS_LABEL, '  meadow   growers market '),
    ];
    const out = mergeAliasValues(KEEP, DROP, statements, 'Meadow Growers Market');
    // The case/space variant must not be recorded as a distinct name. The dropped IRI's own
    // slug ("meadow farmers market") legitimately still is one, so this asserts the variant is
  ...
  it('recovers the name implied by the dropped IRI even with no rdfs:label', () => {
    const statements = [st(KEEP, RDFS_LABEL, 'Meadow Growers Market')];
    // The dropped node was only ever identified by its IRI slug; that is still a name.
    expect(mergeAliasValues(KEEP, DROP, statements, 'Meadow Growers Market').join('|')).toMatch(
      /meadow.farmers.market/i,
    );
  });
  ...
  it("keeps the KEPT entity's own name when the user picks the dropped one as primary", () => {
    ...
    expect(mergeAliasValues(KEEP, DROP, statements, 'Meadow Farmers Market')).toContain(
      'Meadow Growers Market',
    );
  });
  ...
  it('carries aliases the dropped entity had already accumulated (transitive merges)', () => {
    ...
    const out = mergeAliasValues(KEEP, DROP, statements, 'Meadow Growers Market');
    expect(out).toContain('Meadow Farmers Market');
...
describe('newAliasValues', () => {
  it('omits aliases the kept entity already records, so re-merging adds nothing', () => {
    const statements = [
      st(KEEP, RDFS_LABEL, 'Meadow Growers Market'),
      st(KEEP, SKOS_ALT_LABEL, 'Meadow Farmers Market'),
      st(DROP, RDFS_LABEL, 'Meadow Farmers Market'),
    ];
    expect(newAliasValues(KEEP, DROP, statements, 'Meadow Growers Market')).not.toContain(
      'Meadow Farmers Market',
    );
  });
});
...
describe('primaryLabelAfterMerge', () => {
  it('reports the surviving label once the rejected one is excluded', () => {
    const a = st(KEEP, RDFS_LABEL, 'Meadow Growers Market');
    const b = st(KEEP, RDFS_LABEL, 'Meadow Farmers Market');
    expect(primaryLabelAfterMerge(KEEP, [a, b], new Set([b.id]))).toBe('Meadow Growers Market');
    expect(primaryLabelAfterMerge(KEEP, [a, b], new Set([a.id]))).toBe('Meadow Farmers Market');
  });
});
...
describe('buildAliasStatements', () => {
  it('emits confirmed skos:altLabel statements carrying the source graph', () => {
    ...
    expect(out[0].o.value).toBe('Meadow Farmers Market');
...
describe('entityAnswersTo', () => {
  it('matches an entity by a preserved synonym, which is the point of keeping them', () => {
    ...
    expect(entityAnswersTo(KEEP, 'meadow farmers market', statements)).toBe(true);

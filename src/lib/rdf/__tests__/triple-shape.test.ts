...
describe('looksLikeProposition', () => {
  it('catches the observed failure', () => {
    // Dictated "Example Archive is an enterprise DAM"; the model emitted the whole sentence as the
    // subject slug and never extracted the relation.
    expect(looksLikeProposition('example-archive-is-an-enterprise-dam')).toBe(true);
  });
...
describe('subjectSwallowedTriple', () => {
  it('catches a subject containing both its predicate and object, with no verb', () => {
    expect(
      subjectSwallowedTriple('example-archive-category-enterprise-dam', 'category', 'enterprise dam'),
    ).toBe(true);
  });

  it('does not fire when the subject is a plain name', () => {
    expect(subjectSwallowedTriple('example-archive', 'is-a', 'Enterprise DAM')).toBe(false);
  });
...
describe('rejectCollapsedTriples', () => {
  it('drops the collapsed triple and keeps the good one, saying which went', () => {
    const out = rejectCollapsedTriples([
      { subject: 'example-archive-is-an-enterprise-dam', predicate: 'is-a', object: 'thing' },
      { subject: 'example-archive', predicate: 'is-a', object: 'Enterprise DAM' },
    ]);

    expect(out.triples).toHaveLength(1);
    expect(out.triples[0].subject).toBe('example-archive');
    expect(out.rejected).toHaveLength(1);
    expect(out.rejected[0].reason).toBe('subject-is-a-proposition');
  });

  it('never repairs by splitting — a parse is not a fact', () => {
    // Deliberate: inventing "example-archive / is-a / enterprise dam" from the bad slug would turn
  ...
  it('rejects a proposition used as a predicate', () => {
    const out = rejectCollapsedTriples([
      { subject: 'example-archive', predicate: 'example-archive-is-a-dam', object: 'x' },
    ]);
  ...
  it('handles a non-string object without throwing', () => {
    const out = rejectCollapsedTriples([
      { subject: 'example-archive', predicate: 'founded', object: 1999 },
    ]);
    expect(out.triples).toHaveLength(1);
  });

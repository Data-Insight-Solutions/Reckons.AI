...
import {
  composePages, describeComposition, partitionPages, type SourceEntity,
...
function main(): void {
  ...
  // A page with no declared source is HAND-AUTHORED — listed so alignment can see it, generated
  // by nothing. Partitioned with the same helper `docs-compose.ts` uses: this gate used to pass
  // every page straight in and crash on the first one, so the report that exists to be read
  // BEFORE consolidating was the one thing that could not run.
  const { composable, handAuthored } = partitionPages(website.pages);
  ...
  // Named, not merely subtracted. These pages ARE part of the site; a report that showed only the
  // composed ones would understate the site by four pages and read as if they had been dropped.
  if (handAuthored.length > 0) {
    console.log('');
    console.log(C.dim(`  ${handAuthored.length} hand-authored page(s) — declared in the site graph, `
      + `generated by nothing: ${handAuthored.map((p) => p.title).join(', ')}`));
  }

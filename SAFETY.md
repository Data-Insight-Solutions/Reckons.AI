...
- **Local-first privacy.** Pinned by the build — [`svelte.config.js`](svelte.config.js) uses `adapter-static`, so no server exists to receive anything. Nothing is uploaded anywhere unless *you* connect an integration (a folder, Google Drive, an n8n instance, a Git host). There is no central server that sees your data.
